...
var linkSel = 'h3.r a'
var descSel = 'div.st'; // This must have changed from the original
var itemSel = 'div.g'
var nextSel = 'td.b a span'

var sourceAndDate = 'div.slp' 

var URL = '%s://www.google.%s/search?hl=%s&q=%s&tbm=nws&start=%s&sa=N&num=%s&ie=UTF-8&oe=UTF-8&gws_rd=ssl'
  ...
  request(requestOptions, function (err, resp, body) {
    if ((err == null) && resp.statusCode === 200) {
      ...
      $(itemSel).each(function (i, elem) {
        ...
        var descElem = $(elem).find(google.descSel)
        var infospan = $(elem).find(sourceAndDate)
        ...
        $(infospan).find('div').remove()
        item.infospan = $(infospan).text();

        var info = $(infospan).text().split("-");

        item.source = info[0].trim();
        item.date = info[1].trim();
